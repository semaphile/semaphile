...
const pair = (config) => {
  ...
  return [openMemoryBackend(key, config), openMemoryBackend(key, config)];
};
...
test('peer cooldown releases capacity but preserves shared retry floor and budget debit', async () => {
  ...
  try {
    ...
    await release(a, held, { kind: 'throttle', retryAfterMs: 40 });
    assert.equal((await a.call('inspect', {})).active, 0);
    const admitted = await next;
    assert.ok(admitted.leaseGrantedAt >= reportedAt + 40);
...
test('memory breaker allows one probe and fail-fast ignores a blocked local queue', async () => {
  const backends = pair({ maxConcurrent: 2, recovery: { breaker } });
  const [a, b] = backends;
...
test('another owner cannot retire an expired probe or report success for it', async () => {
  const backends = pair({ maxConcurrent: 1, recovery: { breaker } });
  const [a, b] = backends;
  try {
    const failed = await acquire(a);
    await release(a, failed, { kind: 'service-failure' });
    const probe = await acquire(a, { expirationMs: 10 });
    await delay(15);
    ...
    await release(a, probe, { kind: 'success' });
    assert.equal(
      (await control(b)).recovery.circuit,
      'open',
      'expired evidence cannot prove recovery',
    );
...
test('drain tracks expired implicit work until its actual completion', async () => {
  const backends = pair({ maxConcurrent: 1 });
  const [a, b] = backends;
  try {
...
test('unconfirmed memory work needs acknowledgement and never becomes proven complete', async () => {
  const backends = pair({ maxConcurrent: 1 });
  const [a, b] = backends;
  try {
    await acquire(a);
...
test('invalid memory outcomes roll back before release and permit correction', async () => {
  const backends = pair({ maxConcurrent: 1 });
  const [a] = backends;
  try {
    const held = await acquire(a);
...
test('drain and acknowledged uncertainty persist across all clients closing', async () => {
  ...
  const first = openMemoryBackend(key, config);
  await acquire(first);
  const initial = await control(first, { action: 'drain' });
  await first.detach();
  const second = openMemoryBackend(key, config);
  try {
  ...
  const third = openMemoryBackend(key, config);
  try {
